...
#### 初始化
  ...
  对一个类的**主动引用**， 除此之外，所有引用类的方式都不会触发初始化，称为**被动引用**，只有在初始化阶段，才真正开始执行类中定义的Java程序代码（或字节码），初始化阶段是执行类构造器 clinit() 方法的过程，clinit()方法是由编译器自动收集类中的所有**类变量的赋值动作和静态代码块static{}中的语句**合并产生的，其中编译器收集的顺序是由语句在源文件中出现的顺序所决定。
